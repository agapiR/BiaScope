...
sys.path.insert(0, './../')
from utilities_with_interaction import (load_network,
...
overview_layout = html.Div(
    children=[
                ...
                html.Div([
                    ...
                    children = [
                        dcc.Loading(
                            ...
                            type="circle",
                            color="#800020",
                            children=dcc.Graph(id='overview-graph-left', config={'displayModeBar': False}),
                        )
                    ],
        ...
        dcc.Store(id='node-selection-left', data=[], storage_type='memory'),
        dcc.Store(id='node-selection-right', data=[], storage_type='memory'),
...
# get graph selection
@callback(
    Output('node-selection-right', 'data'),
    State('graph-data-overview', 'data'),
    Input('loading-graph-overviewContainerRight', 'children')
)
def store_node_selection_left(graph_data, netRight):
    selectionRight = netRight
    #print(selectionRight)
    # create json object from dictionary
    file = json.dumps(selectionRight)
    ...
    selectedpoints = np.array(range(nx.number_of_nodes(G)))
    # if selectionRight and selectionRight['points']:
    #     selectedpoints = np.array([p['pointIndex'] for p in selectionRight['points']])   
    print(selectedpoints.size)
    
    return list(selectedpoints)


@callback(
    Output('node-selection-left', 'data'),
    Input('overview-graph-left', 'selectedData'),
    Input('graph-data-overview', 'data')
)
def store_node_selection_left(selectionLeft, graph_data):
    G = nx.from_dict_of_dicts(graph_data)
    selectedpoints = np.array(range(nx.number_of_nodes(G)))
    if selectionLeft and selectionLeft['points']:
        selectedpoints = np.array([p['pointIndex'] for p in selectionLeft['points']])
...
@callback(
    ...
    Output('overview-graph-left', 'figure'),
    [Input('graph-data-overview', 'data'),
    Input('networkDropdown', 'value'),
    Input('embDropdownLeft', 'value'),
    Input('fairnessNotion', 'value'),
